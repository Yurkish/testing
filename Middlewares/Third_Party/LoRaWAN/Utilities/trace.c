...
int32_t TraceSend( const char *strFormat, ...)
{
  ...
  if ((status==0 ) && (TracePeripheralReady==SET))
  {
    ...
    TracePeripheralReady = RESET; // if changed to SET UART works wile queu is not full
  ...
  Trace_TxCpltCallback(); // my code. If call this function non DMA transmit works properly.
  return status;
}
